...
public class MainController {
    ...
    @GetMapping("/post")
    public Post getPostByParam(@RequestParam("postId") int postId) {
    ...
    @PutMapping("/posts/{id}")
    public List<Post> updatePost(@PathVariable("id") int id, @RequestBody Post post) {
        Post findPost = posts.stream().filter(post1 -> post1.getId() == id).findFirst().get();
        int indexOf = posts.indexOf(findPost);
        posts.set(indexOf, post);
        return posts;
    }

    @DeleteMapping("/posts/{id}")
    public List<Post> deletePost(@PathVariable("id") int id) {
        posts.removeIf(next -> next.getId() == id);
        return posts;
    }
